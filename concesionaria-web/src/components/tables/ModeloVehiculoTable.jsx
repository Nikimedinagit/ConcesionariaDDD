...
import ModeloVehiculoFiltros, { ModeloVehiculoFiltrosButton } from "@/components/filtros/ModeloVehiculoFiltros";
...
const ModeloVehiculoTable = ({ data, modelosDisponibles, tipo, onToggle, onSearch, onEdit, onToggleStatus, marcas, tiposVehiculos, marcaVehiculoId, tipoVehiculoId, onMarcaChange, onTipoChange }) => {
  ...
  const tiposPorId = useMemo(() => new Map(tiposVehiculos.map((item) => [item.tipoVehiculoId, item.nombre])), [tiposVehiculos]);
  const marcasDisponibles = useMemo(() => {
    const ids = new Set(modelosDisponibles.map((modelo) => modelo.marcaVehiculoId));
  ...
  const tiposDisponibles = useMemo(() => {
    const ids = new Set(modelosDisponibles.map((modelo) => modelo.tipoVehiculoId));
    if (tipoVehiculoId !== "todos") ids.add(tipoVehiculoId);
    return tiposVehiculos.filter((tipoVehiculo) => ids.has(tipoVehiculo.tipoVehiculoId));
  ...
  return (
    <DataTable
      ...
      onToggle={onToggle}
      onSearch={onSearch}
      toolbarActions={<ModeloVehiculoFiltrosButton isOpen={filtersOpen} activeCount={activeFiltersCount} onToggle={() => setFiltersOpen((current) => !current)} />}
      filters={filtersOpen && <ModeloVehiculoFiltros marcaVehiculoId={marcaVehiculoId} tipoVehiculoId={tipoVehiculoId} marcas={marcasDisponibles} tiposVehiculos={tiposDisponibles} onMarcaChange={onMarcaChange} onTipoChange={onTipoChange} onClear={() => { onMarcaChange("todos"); onTipoChange("todos"); }} />}
    />
  );
};
