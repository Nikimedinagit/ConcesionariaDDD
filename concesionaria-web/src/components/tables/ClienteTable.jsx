...
const ClienteTable = ({ data, tipo, onToggle, onSearch, onEdit, onToggleStatus, localidades = [] }) => {
  const localidadPorId = useMemo(() => new Map(localidades.map((localidad) => [localidad.id, localidad.nombre])), [localidades]);
  const columns = useMemo(() => [
    ...
    { accessorKey: "localidadId", header: "Localidad", cell: ({ row }) => localidadPorId.get(row.original.localidadId) || "—" },
  ...
  return <DataTable columns={columns} data={data} tipo={tipo} onToggle={onToggle} onSearch={onSearch} />;
};
