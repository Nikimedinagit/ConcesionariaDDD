          ...
          >
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50/50 p-3 sm:px-4">
              <Button
                ...
                onClick={onClose}
                disabled={loading}
                className="h-9 rounded-xl border-slate-200 px-6 font-semibold text-slate-500 hover:bg-slate-200 hover:text-slate-900"
              >
                {cancelText}
              </Button>
              <Button
                onClick={onSave}
                disabled={loading}
                className="h-9 rounded-xl px-6 font-semibold text-white transition-all hover:opacity-90 active:scale-95 disabled:opacity-50"
                style={{ backgroundColor: "hsl(var(--nav-bg))" }}
              >
                {loading ? "Procesando..." : saveText}
              </Button>
            </div>
          </motion.div>
        </div>
